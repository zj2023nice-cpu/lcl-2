  ...
  if (!canViewArchived) {
    return (
      <Card className="p-12 text-center">
        ...
        <p className="text-theme-text-muted mb-4">仅管理员可查看归档线路</p>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-theme-text-muted mt-1">管理已归档的线路，支持筛选和恢复</p>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="space-y-4">
          {[1, 2, 3, 4].map((i) => (
            <Card key={i} className="p-5 animate-pulse">
              <div className="h-5 bg-theme-border rounded w-1/3 mb-4" />
        ...
        <Card className="p-12 text-center">
          <Archive size={48} className="mx-auto text-theme-text-muted mb-4" />
          <h3 className="text-lg font-medium text-theme-text mb-2">暂无归档线路</h3>
          <p className="text-theme-text-muted">没有找到符合筛选条件的归档线路</p>
        </Card>
      ) : (
        <div className="space-y-4">
          {routes.map((route) => (
            <Card key={route.id} className="overflow-hidden hover:border-theme-border transition-colors">
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-2">
                        <h3 className="font-semibold text-theme-text text-lg">{route.name}</h3>
                        ...
                        <span className="px-2 py-0.5 bg-theme-hover rounded text-xs font-bold text-climbing-orange-400">
                          {route.grade}
                        </span>
                      </div>
                      <div className="space-y-2 text-sm">
                        <div className="p-3 bg-theme-subtle rounded-lg">
                          <div className="flex items-start gap-2">
                            <Archive size={14} className="text-purple-400 flex-shrink-0 mt-0.5" />
                            <div>
                              <p className="text-theme-text-muted text-xs mb-1">归档原因</p>
                              <p className="text-theme-text">{route.archiveReason || '-'}</p>
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <Link
                        to={`/routes/${route.id}`}
                        className="inline-flex items-center gap-1 px-3 py-2 bg-theme-hover hover:bg-theme-border text-theme-text text-sm rounded-lg transition-colors"
                      >
                        查看详情
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 bg-theme-subtle rounded-lg space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-theme-text font-medium">{restoreRoute?.name}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-theme-text-muted">线路定级</span>
              <span className="text-theme-text font-medium">{restoreRoute?.grade}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-theme-text-muted">归档原因</span>
              <span className="text-theme-text text-right max-w-[60%]">{restoreRoute?.archiveReason}</span>
            </div>
            {restoreRoute?.archivedAt && (
              <div className="flex justify-between text-sm">
                <span className="text-theme-text-muted">归档时间</span>
                <span className="text-theme-text">{formatDateTime(restoreRoute.archivedAt)}</span>
              </div>
            )}
          </div>
